...
public class ObstacleFactory {
    /**
     * Default obstacle to return when id is not found
     */
    private static final Obstacle DEFAULT_OBSTACLE = new Obstacle("INDESTRUCTIBLE", ElementType.INDESTRUCTIBLE, List.of(ObstacleTag.PULLABLE_ROPE, ObstacleTag.HERO_HIT_BY_BAT_WILL_BE_STUNNED));
    ...
    /**
     * Find obstacle by id.
     * If id is not found in the map, return default obstacle.
     *
     * @param id String to find obstacle.
     * @return Obstacle mapped with id, or default obstacle if not found.
     */
    public static Obstacle getObstacleById(String id){
        Obstacle obstacle = obstacleMap.get(id);
        if (obstacle == null) {
